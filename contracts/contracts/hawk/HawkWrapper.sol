...
/// @title HawkWrapper
/// @notice ERC-1155 wrapping, fuses, and subdomain permissions for .hawk
...
///      1. TLD constants: HAWK_NODE = namehash('hawk'), HAWK_LABELHASH =
///         keccak256('hawk'), and the DNS-encoded suffix "\x04hawk\x00"
///         (two sites: constructor and _wrapETH2LD).
///      2. GRACE_PERIOD is immutable, read from the Hawk base registrar at
///         deploy (mainnet: 90 days, the upstream value) so testnet lifecycle
///         rehearsals can run with shortened timers.
///      3. Collection name: "Hawk Wrapped Names".
///      4. Doc comments reference .hawk instead of .eth.
contract HawkWrapper is
    Ownable,
    ERC1155Fuse,
    INameWrapper,
    Controllable,
    ...
    ReverseClaimer
{
    ...
    string public constant name = "Hawk Wrapped Names";

    // Hawk diff: immutable, read from the registrar at deploy so the two
    ...
    bytes32 private constant HAWK_LABELHASH =
    ...
    constructor(
        ...
        GRACE_PERIOD = uint64(
            HawkBaseRegistrar(address(_registrar)).GRACE_PERIOD()
        );
        metadataService = _metadataService;

        /* Burn PARENT_CANNOT_CONTROL and CANNOT_UNWRAP fuses for ROOT_NODE and HAWK_NODE and set expiry to max */
    ...
    /// @param id Label as a string of the .hawk domain to wrap
    ...
    /// @notice Wraps a .hawk domain, creating a new token and sending the original ERC721 token to this contract
    /// @dev Can be called by the owner of the name on the .hawk registrar or an authorised caller on the registrar
    /// @param label Label as a string of the .hawk domain to wrap
    ...
    function wrapETH2LD(
        ...
        if (
            ...
            revert Unauthorised(
                _makeNode(HAWK_NODE, bytes32(tokenId)),
                msg.sender
            );
        }
    ...
    /// @dev Registers a new .hawk second-level domain and wraps it.
    ///      Only callable by authorised controllers.
    /// @param label The label to register (Eg, 'foo' for 'foo.hawk').
    /// @param wrappedOwner The owner of the wrapped name.
    /// @param duration The duration, in seconds, to register the name for.
    /// @param resolver The resolver address to set on the ENS registry (optional).
    /// @param ownerControlledFuses Initial owner-controlled fuses to set
    /// @return registrarExpiry The expiry date of the new name on the .hawk registrar, in seconds since the Unix epoch.
    ...
    /// @notice Renews a .hawk second-level domain.
    /// @dev Only callable by authorised controllers.
    /// @param tokenId The hash of the label to register (eg, `keccak256('foo')`, for 'foo.hawk').
    /// @param duration The number of seconds to renew the name for.
    /// @return expires The expiry date of the name on the .hawk registrar, in seconds since the Unix epoch.
    function renew(
        uint256 tokenId,
        uint256 duration
    ) external onlyController returns (uint256 expires) {
        bytes32 node = _makeNode(HAWK_NODE, bytes32(tokenId));
    ...
    /// @notice Wraps a non .hawk domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    ...
    /// @notice Unwraps a .hawk domain. e.g. vitalik.hawk
    /// @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    /// @param labelhash Labelhash of the .hawk domain
    /// @param registrant Sets the owner in the .hawk registrar to this address
    /// @param controller Sets the owner in the registry to this address
    function unwrapETH2LD(
        bytes32 labelhash,
        address registrant,
        address controller
    ) public onlyTokenOwner(_makeNode(HAWK_NODE, labelhash)) {
        if (registrant == address(this)) {
            revert IncorrectTargetOwner(registrant);
        }
        _unwrap(_makeNode(HAWK_NODE, labelhash), controller);
    ...
    /// @notice Unwraps a non .hawk domain, of any kind. Could be a DNSSEC name vitalik.xyz or a subdomain
    /// @dev Can be called by the owner in the wrapper or an authorised caller in the wrapper
    ...
    /// @notice Upgrades a domain of any kind. Could be a .hawk name vitalik.hawk, a DNSSEC name vitalik.xyz, or a subdomain
    ...
    function _beforeTransfer(
        ...
        // For this check, treat .hawk 2LDs as expiring at the start of the grace period.
        if (fuses & IS_DOT_ETH == IS_DOT_ETH) {
            expiry -= GRACE_PERIOD;
        }
    ...
    function _normaliseExpiry(
        ...
        // .hawk names will check registrar, non .hawk check parent
        if (expiry > maxExpiry) {
            expiry = maxExpiry;
        }
    ...
    function _wrapETH2LD(
        ...
        bytes32 labelhash = keccak256(bytes(label));
        bytes32 node = _makeNode(HAWK_NODE, labelhash);
        // hardcode dns-encoded hawk string for gas savings
        bytes memory name = _addLabel(label, "\x04hawk\x00");
        names[node] = name;
